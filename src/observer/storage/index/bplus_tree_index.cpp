...
RC BplusTreeIndex::create(const char *file_name, const bool unique, const IndexMeta &index_meta, const std::vector<int> &field_ids, const std::vector<const FieldMeta*> &field_metas)
{
  if (inited_) {
  ...
  RC rc = index_handler_.create(file_name, unique, field_ids, field_metas);
  if (RC::SUCCESS != rc) {
