...
const static Json::StaticString FIELD_FIELD_AMOUNT("field_amount");
...
void IndexMeta::to_json(Json::Value &json_value) const
{
  ...
  json_value[FIELD_FIELD_AMOUNT] = field_.size();
  Json::Value fields;
...
RC IndexMeta::from_json(const TableMeta &table, const Json::Value &json_value, IndexMeta &index)
{
  ...
  const Json::Value &field_amount = json_value[FIELD_FIELD_AMOUNT];
  ...
  if (!field_amount.isInt()) {
    LOG_ERROR("Field num of index [%s] is not a integer. json value=%s",
        name_value.asCString(),
        field_amount.toStyledString().c_str());
        return RC::INTERNAL;
  }

  if (field_amount.asInt() != (int)field_value.size()) {
  ...
  for (int i = 0; i < field_amount.asInt(); i++) {
