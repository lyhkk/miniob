...
RC BplusTreeHandler::create(const char *file_name, const bool unique, const std::vector<int> &field_ids, const std::vector<const FieldMeta*> &fields, 
    int internal_max_size /* = -1*/, int leaf_max_size /* = -1 */)
{
  ...
  file_header->attr_amount = fields.size();
  ...
  key_comparator_.init(file_header->unique, file_header->attr_amount, file_header->field_id, file_header->attr_type, file_header->attr_length);
  key_printer_.init(file_header->attr_amount, file_header->attr_type, file_header->attr_length);
...
RC BplusTreeHandler::open(const char *file_name)
{
  ...
  key_comparator_.init(file_header_.unique, file_header_.attr_amount, file_header_.field_id, file_header_.attr_type, file_header_.attr_length);
  key_printer_.init(file_header_.attr_amount, file_header_.attr_type, file_header_.attr_length);
...
MemPoolItem::unique_ptr BplusTreeHandler::make_key(const char *user_key, const RID &rid)
{
  ...
  //复制索引列
  for (int i = 1; i < file_header_.attr_amount; i++) {
...
RC BplusTreeHandler::delete_entry(const char *user_key, const RID *rid)
{
  ...
  memcpy(key + offset, rid, rid_size);
